...
function pfrCandidateGaps(lines, pageHeight, previousTail = "") {
  ...
  for (let i = 0; i < lines.length - 1; i += 1) {
    ...
    const beforeLines = lines.slice(Math.max(0, i - 6), i + 1);
    const afterLines = lines.slice(i + 1, Math.min(lines.length, i + 4));
...
async function pfrRenderGap(page, gap) {
  const scale = 1.7;
...
function pfrInsertionPoint(section, gap, number) {
...
function pfrExisting(section, pageNumber, number, gapIndex) {
  ...
  if (section.querySelector(`img[data-pdf-recovery-v346="${pageNumber}-${gapIndex}"]`)) return true;
  return false;
}
...
function pfrInsert(section, dataUrl, fileName, pageNumber, gap, gapIndex) {
  if (pfrLooksLikeStartBanner(pageNumber, gap)) return null;
  const number = gap.labelNumber || pfrFigureNumber(`${gap.before} ${gap.afterContext || ""}`);
  if (pfrExisting(section, pageNumber, number, gapIndex)) return null;
  ...
  image.dataset.pdfRecoveryV346 = `${pageNumber}-${gapIndex}`;
...
function pfrAudit(inserted) {
  ...
  li.dataset.pdfRecoveryAudit = "v346";
  ...
  if (inserted > 0) {
    ...
    if (status) {
      status.textContent = `PDF v${PDF_FIGURE_RECOVERY_VERSION}: ${inserted} figura(s) adicional(es) incorporada(s) desde el documento original.`;
...
async function recoverPdfFiguresV346() {
  ...
  try {
    for (const file of files) {
      ...
      if (!section || section.dataset.pdfFigureRecoveryV346 === "true") continue;
      inserted += await pfrRecoverFile(file, section);
      section.dataset.pdfFigureRecoveryV346 = "true";
    }
    ...
    console.error("PDF figure recovery v3.4.6", error);
    const status = pfrQ("#status");
    if (status) {
...
function pfrSchedule(delay = 700) {
  clearTimeout(pfrTimer);
  pfrTimer = setTimeout(() => void recoverPdfFiguresV346(), delay);
}
...
function pfrInitialize() {
  ...
  pfrQ("#reauditBtn")?.addEventListener("click", () => pfrSchedule(350));
  pfrSchedule(1100);
}

window.recoverPdfFiguresV346 = recoverPdfFiguresV346;
